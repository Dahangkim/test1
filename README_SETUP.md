...
활동가 위치정보는 활동가가 `내 위치 첨부` 버튼을 직접 눌러 브라우저 위치 권한을 허용한 경우에만 접수됩니다. 위치정보는 관리자 검토용으로만 사용하고 공개 대시보드에는 표시하지 마세요.
...
- 위치정보는 개인의 이동·현장활동을 드러낼 수 있으므로 수집 필요성, 보존 기간, 접근 권한을 별도로 정하세요.
